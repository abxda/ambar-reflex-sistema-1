...
> Hice un clon local de Jev (decisiones tipadas, sin generar texto) y lo puse a jugar un run-and-gun ochentero: 408 ms por jugada y 24.9 % del nivel. El mismo modelo escribiendo JSON: 1041 ms y 10.3 %. Código abierto 👇 — Dr. Coronado (@abxda)

**Hilo:**

1/ Jev (TypeSafe AI) es un modelo «Sistema 1»: no escribe, decide. Le das un estado y preguntas cerradas,
y te devuelve probabilidades. Armé un clon local compatible con su API: Qwen3.5-4B en una RTX 3060.
— Dr. Coronado (@abxda)

2/ Para medirlo en serio hice un juego original de 8 bits. En cada jugada, una sola petición con 6
preguntas: mover, saltar, agacharse, apuntar, disparar y peligro. 408 ms, 0 tokens generados.
— Dr. Coronado (@abxda)

3/ Lo que funcionó: disparar acierta el 97 %. Lo que no: «saltar» tiene un sesgo y apenas 18 % de
precisión. Y la redacción importa más que el modelo: sin decirle la meta, se quedó quieto para siempre.
— Dr. Coronado (@abxda)

4/ Describir dónde estarán las balas cuando llegue la respuesta (compensación de latencia) subió el
avance de 15.6 % a 26.2 %. El formato está garantizado; la corrección no.
...
disponible como API en la nube, así que construí un **clon local compatible con su API**. Usa el método
abierto de lectura de probabilidades (SemIf) sobre Qwen3.5-4B, con un motor llama.cpp optimizado, y corre
en una RTX 3060.
